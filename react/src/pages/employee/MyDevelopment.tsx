      ...
      <Card p={false}><div className="border-b p-6" style={{ borderColor: BD }}><SectionHead title="My Development Plans" subtitle="Create employee-owned objectives and keep roadmap actions current" /></div>
        <form onSubmit={submitPlan} className="grid gap-4 p-6 md:grid-cols-2">
          ...
          <div className="flex items-end">
            <PrimaryBtn type="submit" disabled={saving} icon={Plus}>Create development plan</PrimaryBtn>
          </div>
        </form>
        {plans.length === 0 ? <div className="border-t" style={{ borderColor: BD }}><div className="p-10 text-center"><p className="text-sm font-medium" style={{ color: TX }}>No development plan yet</p><p className="mt-2 text-sm" style={{ color: TX2 }}>Create an objective above to start your development roadmap.</p></div><div className="border-t p-6" style={{ borderColor: BD }}><h3 className="text-sm font-semibold" style={{ color: TX }}>Development roadmap</h3><p className="mt-1 text-xs" style={{ color: TX2 }}>Activities and action steps will appear here after you create a development plan.</p><p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm" style={{ color: TX2 }}>No development activities yet.</p></div></div> : <div className="space-y-4 border-t p-6" style={{ borderColor: BD }}>{plans.map((plan) => {
          ...
          return <div key={plan.id} className="rounded-2xl border p-5" style={{ borderColor: BD }}>
            ...
            <div className="mt-5"><ProgressBar value={plan.progress} /></div>
            <div className="mt-5 grid gap-3 border-t pt-4 md:grid-cols-2" style={{ borderColor: BD }}>
              <label className="text-xs font-medium" style={{ color: TX2 }}>Objective<input maxLength={160} value={draft.title} onChange={(event) => setDrafts({ ...drafts, [plan.id]: { ...draft, title: event.target.value } })} className="mt-1 block w-full rounded-xl border px-3 py-2 text-sm font-normal text-slate-700" style={{ borderColor: BD }} /></label>
              <label className="text-xs font-medium" style={{ color: TX2 }}>Focus area<input maxLength={80} value={draft.focus_area} onChange={(event) => setDrafts({ ...drafts, [plan.id]: { ...draft, focus_area: event.target.value } })} className="mt-1 block w-full rounded-xl border px-3 py-2 text-sm font-normal text-slate-700" style={{ borderColor: BD }} /></label>
              <label className="text-xs font-medium md:col-span-2" style={{ color: TX2 }}>Description<textarea maxLength={4000} value={draft.description} onChange={(event) => setDrafts({ ...drafts, [plan.id]: { ...draft, description: event.target.value } })} className="mt-1 min-h-16 w-full rounded-xl border px-3 py-2 text-sm font-normal text-slate-700" style={{ borderColor: BD }} /></label>
              <label className="text-xs font-medium" style={{ color: TX2 }}>Target date<input type="date" value={draft.target_date} onChange={(event) => setDrafts({ ...drafts, [plan.id]: { ...draft, target_date: event.target.value } })} className="mt-1 block w-full rounded-xl border px-3 py-2 text-sm font-normal text-slate-700" style={{ borderColor: BD }} /></label>
              <label className="text-xs font-medium" style={{ color: TX2 }}>Status<select value={plan.status} onChange={(event) => updatePlan(plan, { status: event.target.value as PlanStatus })} className="mt-1 block w-full rounded-xl border px-3 py-2 text-sm font-normal text-slate-700" style={{ borderColor: BD }}><option value="draft">Draft</option><option value="in_progress">In progress</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></label>
              <label className="text-xs font-medium" style={{ color: TX2 }}>Progress <span className="font-normal">(0–100)</span><input type="number" min="0" max="100" value={plan.progress} onChange={(event) => updatePlan(plan, { progress: clampProgress(event.target.value) })} className="mt-1 block w-full rounded-xl border px-3 py-2 text-sm font-normal text-slate-700" style={{ borderColor: BD }} /></label>
              <label className="text-xs font-medium" style={{ color: TX2 }}>Notes <span className="font-normal">(optional)</span><input maxLength={3000} value={draft.notes} onChange={(event) => setDrafts({ ...drafts, [plan.id]: { ...draft, notes: event.target.value } })} className="mt-1 block w-full rounded-xl border px-3 py-2 text-sm font-normal text-slate-700" style={{ borderColor: BD }} /></label>
              <div className="md:col-span-2"><SecondaryBtn disabled={saving} onClick={() => updatePlan(plan, draft)} icon={Save}>Save plan changes</SecondaryBtn></div>
            </div>
