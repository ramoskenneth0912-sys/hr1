...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '' || mb_strlen($title) > 200) {
        $errors['title'] = 'A title (max 200 characters) is required.';
    }
    if ($jobId <= 0) {
        $errors['job_posting_id'] = 'The associated Job/Position is required.';
    }
    if ($sourceProvider === '' || mb_strlen($sourceProvider) > 80) {
        $errors['source_provider'] = 'A source provider (max 80 characters) is required.';
    }
    if ($externalRef !== '' && mb_strlen($externalRef) > 120) {
        $errors['external_ref'] = 'The external reference may not exceed 120 characters.';
    }
    ...
    if (mb_strlen($description) > 2000) {
        $errors['description'] = 'Description may not exceed 2000 characters.';
    }
    if (mb_strlen($instructions) > 4000) {
        $errors['instructions'] = 'Instructions may not exceed 4000 characters.';
    }
    if ($examUrl !== '') {
    ...
    if ($errors) {
        // No server-side trust of posted values on failure — re-render form with values.
        $exam = [
        ...
        $flashErrors = implode(' ', $errors);
        // fall through to render with an inline error banner
        $renderErrors = $errors;
    } else {
        $stmt = db()->prepare(
            'INSERT INTO exams
                (title, job_posting_id, description, instructions, exam_url, passing_score, time_limit_minutes,
                 max_attempts, source_provider, external_ref, status, created_by)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?)'
        );
        $stmt->execute([
            $title,
        ...
        securityLog('exam_created', "exam_id={$examId} title=" . mb_substr($title, 0, 60), $_SESSION['user_id'] ?? null);

        flash('success', 'Examination reference created. It can now be assigned to eligible applicants.');
        redirect(BASE_URL . '/modules/recruitment/exams/view.php?id=' . $examId);
    }
}
...
<div class="page-header fade-in-up">
    <div>
        ...
        <p class="page-subtitle">Define an HR1-side examination reference (the exam content is provided by an external HR3 system)</p>
    </div>
...
<form method="post" class="form-panel fade-in-up" style="animation-delay:.1s">
    ...
    <div class="form-grid">
        <div class="form-group full-width">
            ...
            <input type="text" id="title" name="title" value="<?= e($exam['title'] ?? '') ?>" required placeholder="e.g. HR Assistant Examination">
        </div>
        <div class="form-group full-width">
            ...
            <select id="job_posting_id" name="job_posting_id" required>
                ...
                <option value="<?= (int) $j['id'] ?>" <?= (int) ($exam['job_posting_id'] ?? 0) === (int) $j['id'] ? 'selected' : '' ?>>
        ...
        <div class="form-group">
            ...
            <input type="text" id="source_provider" name="source_provider" value="<?= e($exam['source_provider'] ?? 'HR3') ?>" placeholder="HR3" required>
            <small style="color:var(--muted)">The external examination provider (HR1 stores only this reference, not the exam content).</small>
        </div>
        <div class="form-group">
            <label for="external_ref">External Provider Reference</label>
            <input type="text" id="external_ref" name="external_ref" value="<?= e($exam['external_ref'] ?? '') ?>" placeholder="Optional — provider's exam id">
            <small style="color:var(--muted)">Optional identifier used by the external provider to identify this exam.</small>
        </div>
        <div class="form-group">
            <label for="passing_score">Passing Score (%)</label>
            <input type="number" id="passing_score" name="passing_score" min="0" max="100" step="0.01" value="<?= e($exam['passing_score'] ?? '60') ?>">
        </div>
        <div class="form-group">
            <label for="time_limit_minutes">Time Limit (minutes)</label>
            <input type="number" id="time_limit_minutes" name="time_limit_minutes" min="1" value="<?= e($exam['time_limit_minutes'] ?? '30') ?>">
        </div>
        <div class="form-group">
            <label for="max_attempts">Max Attempts</label>
            <input type="number" id="max_attempts" name="max_attempts" min="1" value="<?= e($exam['max_attempts'] ?? '1') ?>">
        </div>
        <div class="form-group">
